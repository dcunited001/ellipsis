...
***** TODO finish XDG User Dirs config
+ [ ] xdg-user-dirs-update will replace everything if anything is invalid (only =$HOME/yyy= or =/path/to/yyy= is valid)
  - it will autoupdate everything if anything doesn't exist
+ [ ] determine whether this goes here or in Bash.org
+ [ ] autogenerate this, but determine how the =update-dotfiles= environment will handle this.
...
# export FLATPAK_CONFIG_DIR=/flatpak/...
#+end_src
...
for app_dir in $FLATPAK_USER_VAR/app/*; do
    [[ -L app_dir ]] && ln -s $_DF/.config/fontconfig $app_dir/config/fontconfig
done
#+end_example
