...
guix_arch_load() {
    local extra_profiles=$HOME/.guix-extra-profiles
    local profiles=""
    ...
    if [ -z "$profiles" ]; then
        # NOTE: this glob behaves strangely when $extra_profiles is empty
    ...
    for p in $profiles; do
        profile=$(basename $p)
        echo $profile
        profilePath=$extra_profiles/$profile
...
Add to =~/.config/sh/_load_profile.d.sh=:
...
Activate new environments with:


Pass python to Julia Shells with:
