...
#XDG_DOWNLOAD_DIR="$_DATA/xdg/Downloads"
XDG_DOCUMENTS_DIR="$_DATA/xdg/Documents"
XDG_MUSIC_DIR="$_DATA/xdg/Music"
XDG_PICTURES_DIR="$_DATA/xdg/Pictures"
XDG_VIDEOS_DIR="$_DATA/xdg/Videos"
XDG_TEMPLATES_DIR="$_DATA/xdg/Templates"
...
Guix packs =/etc/configs= into the packages, so customization requires either
CLI, environment or extending the flatpak package. For flatpak, changing this
variable allows for a new custom installation config. Custom installations need
to be configured in =$FLATPAK_CONFIG_DIR=.

#+begin_example sh
# to override the default /etc/flatpak, which is inside the guix package
# - or simply use $FLATPAK_USER_DIR
# FLATPAK_CONFIG_DIR=/flatpak/$(id -un)/.config/flatpak
#+end_example

And, flatpak overrides $XDG_DATA_DIRS anyway, so this won't work

#+begin_example sh
# export XDG_DATA_DIRS="/flatpak/$(id -un)/.local/share/flexpak/exports/share:${XDG_DATA_DIRS}"
#+end_example
