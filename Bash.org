...
export PYENV_PEOTRY_VENV=poetry
...
#+begin_src sh :tangle .bin/pynv-install-poetry
...
#+begin_src toml :tangle .config/pypoetry/config.toml
[virtualenvs]
...
Without any other config, once =poetry= is installed for =$pyversion=, then with
the above function/alias, =ptry config --list= should show:
...
#+begin_src sh :tangle .config/direnv/eg/pyenv
# -*- mode: sh; -*-
[[ -f ".python-version" ]] && pyversion=$(head .python-version) || pyversion="3.10.11"
[[ -f ".python-virtualenv" ]] && pvenv=$(head .python-virtualenv) || pyenv=$(basename $PWD)
...
#+begin_src sh :tangle .config/direnv/eg/poetry
