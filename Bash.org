...
use_julia_system() {
    ...
    PATH_add "$JULIA_PATH/.bin" "$MINIFORGE_BIN"
}
#+end_src
...
# ... so i can just add environment variables, right?
__conda_setup="$('$MINIFORGE_BIN/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "$MINIFORGE_BIN/miniforge3/etc/profile.d/conda.sh" ]; then
        . "$MINIFORGE_BIN/miniforge3/etc/profile.d/conda.sh"
    else
        # make sure this is the very last, last, last thing that runs in your shell. after that other last thing
        export PATH="$MINIFORGE_BIN:$PATH"
    fi
fi
unset __conda_setup
