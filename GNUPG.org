...
#+begin_src shell :tangle (concat (getenv "SECRETS_HOME") "/init.sh") :shebang "#!/bin/sh" :tangle-mode (identity #o500)
export GPG_TTY="$(tty)"
...
#+begin_src shell :tangle (concat (getenv "SECRETS_HOME") "/.bin/start-gpg-connect") :shebang "#!/bin/sh" :tangle-mode (identity #o500)
export GPG_TTY="$(tty)"
unset SSH_AGENT_PID
...
#+begin_src shell :tangle (concat (getenv "SECRETS_HOME") "/.bin/start-gpg") :shebang "#!/bin/sh" :tangle-mode (identity #o500)
export GPG_TTY="$(tty)"
unset SSH_AGENT_PID
export SSH_AUTH_SOCK="$(gpgconf --list-dirs agent-ssh-socket)"
...
#+begin_src conf :tangle (concat (getenv "SECRETS_HOME") "/.gnupg/gpg.conf.defaults")
...
#+begin_src conf :tangle (concat (getenv "SECRETS_HOME") "/.gnupg/gpg-agent.conf.defaults")
...
#+begin_src scheme :tangle (concat (getenv "SECRETS_HOME") "/.shroud.defaults")
...
#+begin_src emacs-lisp :tangle (concat (getenv "SECRETS_HOME") "/.emacs.d/init.el")
