...
keys are created per-message. The subkey ID will be included unless the
=throw-keyid= option is set.
...
+ However, AFAIK, using the =<(process substitution)= creates FD's that expire
  ...
  kernel, i'm done with this". NO FUCKING NODEJS BINARY WILL DO THIS FOR YOU.
