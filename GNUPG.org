...
#+begin_src shell :var gpgmaster=gkmaster gpgsign=gksign gpgcrypt=gkcrypt gpgauth=gkauth :noweb-ref gkring :noweb-sep ""
export GKMASTER=$gpgmaster
export GKSIGN=$gpgsign
export GKCRYPT=$gpgcrypt
export GKAUTH=$gpgauth
#+end_src
...
#guixprofile=$(find $SECRETS_HOME/pkg/gnu/store -wholename "*profile/etc/profile")
...
#+begin_src shell :tangle (concat (or (getenv "SECRETS_HOME") ".") "/.gnupg/init.sh") :shebang "#!/bin/sh" :tangle-mode (identity #o500) :noweb yes
<<gkring>>
#+end_src

#+begin_src shell :tangle (concat (or (getenv "SECRETS_HOME") ".") "/.gnupg/init.sh") :tangle-mode (identity #o500)
export GPG_TTY="$(tty)"
